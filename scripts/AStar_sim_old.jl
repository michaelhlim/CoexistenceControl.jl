...
if parallelize
	...
	Distributed.@everywhere begin
		...
		if "SLURM_JOBID" in keys(ENV)
			Distributed.@everywhere directory_string = "./scripts/helper"
...
try
	...
	for reps = 1:total_replicates
		...
		params_astar = get_data(
			n_species, n_temps, A_sampler, r_sampler, portion, scaling, 
		...
		(assemblage, transitions_natural, graph_astar) = get_assemblage(
		...
		results_table = run_AStar(
			n_species, n_temps, params_astar, assemblage, graph_astar, 
